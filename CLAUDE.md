...
> 最后更新：2026-09-22（1.17 把「下载日志」做进主界面按钮，MuMu 16 项实测通过）
...
| **1.17（当前）** | 把「下载日志」做成**主界面按钮**（放「方控」左侧），去掉 1.16 多出的独立桌面图标 | **MuMu 16 项通过；车机实测待做** |

**1.17 成品**：`CarLife/05_产物/CarLife4.0车机端个人修改版1.17_日志下载入口.apk`
（2,610,425 字节，versionCode 117，签名 v1+v2+v3）
...
**1.16 / 1.17 还没被真机验过的**：
1. 车机主界面上「下载日志」按钮的实际排布/字号（车机屏尺寸与 MuMu 不同，MuMu 走的是 `layout-hdpi`）
2. 车机有没有物理返回键（没有就用下载页上新加的「返回」按钮）
3. 车机真实网卡下的 IP（直连应为 `192.168.49.1`，热点为手机分配的 `192.168.x.x`）
4. 真手机用相机扫码（MuMu 无相机，改用「截图 → 验二维码.py 解码 → 按解出的 URL 下载」等价验证）
5. 车机 ROM 若精简了 WebView，二维码不显示（会退化为纯文字地址，功能仍可用）
...
│   ├── 01_工程源码\
│   │   └── _w111 .. _w117\      1.11 ~ 1.17 各版本工程树（_w117 = 当前）
│   ├── 02_补丁脚本\
