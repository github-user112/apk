# -*- coding: utf-8 -*-
"""
CarLife 车机端 (com.baidu.carlifevehicle, 4.0.M1.3-mod1.6) 直连修复补丁 v2
...
inject_c = (
    '    iput-object v1, p0, La/a/a/a/m/m/d/i;->e:La/a/a/a/m/m/d/h;\n'
    '\n'
    '    # --- patchC: 把系统 P2P 设备名同步为配置的直连名称, 手机端才能按名字找到车机 ---\n'
    '    # ---         配置缺失/为空时回退默认值, 避免上报空名给手机端 ---\n'
...
write_text(p, s)
log('[C] i.smali <init> -> setDeviceName(名称为空时回退 CarLife-HU)')
...
def check():
    ...
    for k in ('setDeviceName', ':p2p_name_set', 'if-nez v1, :p2p_name_len'):
        if k not in i:
            log('!! i.smali 缺少 %s' % k); ok = False
    if 'if-nez v1, :p2p_name_ok' in i or 'bt_auto_enable\n\n    invoke-virtual {v5}, Landroid/bluetooth/BluetoothAdapter;->enable' not in a:
        log('!! 检测到旧版补丁残留'); ok = False
