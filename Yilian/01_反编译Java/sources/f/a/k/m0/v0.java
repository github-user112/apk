package f.a.k.m0;

/* loaded from: classes.dex */
public class v0 implements f.a.k.k0.i, f.a.i.h.c, f.a.i.h.g {
    public static final java.lang.Object h = new java.lang.Object();
    public final f.a.k.k0.j a;
    public final android.os.Handler b;

    /* renamed from: c, reason: collision with root package name */
    public final f.a.i.j.c f3106c;

    /* renamed from: e, reason: collision with root package name */
    public java.lang.String f3108e;

    /* renamed from: d, reason: collision with root package name */
    public boolean f3107d = true;

    /* renamed from: f, reason: collision with root package name */
    public boolean f3109f = false;

    /* renamed from: g, reason: collision with root package name */
    public boolean f3110g = false;

    public v0(f.a.k.k0.j jVar, android.os.Handler handler, f.a.i.j.c cVar) {
        this.a = jVar;
        f.a.k.l0.a4 a4Var = (f.a.k.l0.a4) jVar;
        if (a4Var == null) {
            throw null;
        }
        a4Var.x = this;
        this.b = handler;
        this.f3106c = cVar;
    }

    @Override // f.a.i.h.c
    public void A() {
    }

    @Override // f.a.i.h.c
    public void C() {
    }

    @Override // f.a.i.h.c
    public void D() {
    }

    @Override // f.a.i.h.c
    public void F() {
    }

    @Override // f.a.i.h.c
    public void G(int i) {
        e.e.a.g.c(e.a.c.a.a.K("onConfigFailed : type = ", i), new java.lang.Object[0]);
        r0();
        this.f3106c.a(com.google.android.exoplayer2.audio.Ac3Util.AC3_MAX_RATE_BYTES_PER_SECOND, 20, java.lang.String.valueOf(i));
    }

    @Override // f.a.i.h.c
    public void J() {
    }

    @Override // f.a.i.h.g
    public void M(int i, java.lang.String str) {
    }

    @Override // f.a.i.h.c
    public void P(int i, java.lang.String str) {
    }

    @Override // f.a.i.h.c
    public void Q(java.lang.String str, java.lang.String str2, java.lang.String str3, java.lang.String str4, java.lang.String str5, java.lang.String str6, java.lang.String str7, java.lang.String str8, java.lang.String str9) {
    }

    @Override // f.a.i.h.g
    public void S(java.lang.String str, java.lang.String str2, java.lang.String str3, java.lang.String str4, int i) {
    }

    @Override // f.a.i.h.c
    public void Z(int i) {
        e.e.a.g.c(e.a.c.a.a.K("onTransportFailed : type = ", i), new java.lang.Object[0]);
        this.b.removeCallbacksAndMessages(h);
        r0();
        this.f3106c.a(com.google.android.exoplayer2.audio.Ac3Util.AC3_MAX_RATE_BYTES_PER_SECOND, 20, java.lang.String.valueOf(i));
    }

    @Override // f.a.k.k0.i
    public void a(int i, int i2, java.lang.String str) {
        f.a.i.j.c cVar = this.f3106c;
        if (cVar != null) {
            cVar.a(i, i2, str);
        }
    }

    @Override // f.a.k.k0.i
    public boolean a0() {
        return this.f3109f;
    }

    @Override // f.a.i.h.g
    public void c() {
    }

    @Override // f.a.i.h.c
    public void c0(java.util.List<net.easyconn.ecsdk.ECTypes.ECPageInfo> list) {
    }

    @Override // f.a.i.h.c
    public void d() {
        android.os.Handler handler = this.b;
        final f.a.k.k0.j jVar = this.a;
        jVar.getClass();
        handler.post(new java.lang.Runnable() { // from class: f.a.k.m0.i
            @Override // java.lang.Runnable
            public final void run() {
                jVar.d();
            }
        });
    }

    public void e(boolean z, net.easyconn.ecsdk.ECTypes.ECTransportType eCTransportType) {
        e.e.a.g.c("onPhoneConnectStatus : isConnected = " + z, new java.lang.Object[0]);
        this.b.removeCallbacksAndMessages(h);
        this.f3109f = z;
        if (z) {
            y0();
            return;
        }
        android.os.Handler handler = this.b;
        f.a.k.k0.j jVar = this.a;
        jVar.getClass();
        handler.post(new f.a.k.m0.o0(jVar));
    }

    public void e0() {
    }

    @Override // f.a.i.h.c
    public void f(boolean z) {
    }

    @Override // f.a.i.h.c
    public void f0(boolean z) {
    }

    @Override // f.a.i.h.c
    public void g(long j) {
    }

    @Override // f.a.i.h.c
    public void h(int i) {
        f.a.i.j.c cVar = this.f3106c;
        if (cVar != null) {
            cVar.a(com.google.android.exoplayer2.audio.Ac3Util.AC3_MAX_RATE_BYTES_PER_SECOND, 20, java.lang.String.valueOf(i));
        }
        e.e.a.g.c(e.a.c.a.a.K("onInitFailed : type = ", i), new java.lang.Object[0]);
        r0();
    }

    @Override // f.a.i.h.c
    public void h0(net.easyconn.ecsdk.ECCustomProtocol.ECCustomData eCCustomData) {
    }

    @Override // f.a.i.h.c
    public void i(net.easyconn.ecsdk.ECTypes.ECPageInstallResult eCPageInstallResult) {
    }

    @Override // f.a.i.h.g
    public void j(java.lang.String str, float f2, int i, int i2) {
    }

    @Override // f.a.i.h.c
    public void j0() {
        this.b.removeCallbacksAndMessages(h);
        android.os.Handler handler = this.b;
        f.a.k.k0.j jVar = this.a;
        jVar.getClass();
        handler.post(new f.a.k.m0.o0(jVar));
    }

    @Override // f.a.i.h.c
    public void m(boolean z) {
    }

    @Override // f.a.i.h.c
    public void m0(byte[] bArr, int i) {
    }

    @Override // f.a.i.h.c
    public void n0(net.easyconn.ecsdk.ECTypes.ECTransportType eCTransportType) {
    }

    @Override // f.a.i.h.c
    public void o(int i, int i2, int i3) {
    }

    @Override // f.a.c
    public void onDestroy() {
        f.a.i.j.c cVar = this.f3106c;
        if (cVar != null) {
            cVar.g();
        }
        this.b.removeCallbacksAndMessages(null);
        f.a.k.l0.a4 a4Var = (f.a.k.l0.a4) this.a;
        if (a4Var == null) {
            throw null;
        }
        a4Var.x = null;
    }

    @Override // f.a.i.h.c
    public void onLicenseAuthFail(final int i, final java.lang.String str) {
        this.b.post(new java.lang.Runnable() { // from class: f.a.k.m0.c0
            @Override // java.lang.Runnable
            public final void run() {
                this.a.u0(i, str);
            }
        });
    }

    @Override // f.a.i.h.c
    public void onLicenseAuthSuccess(final int i, final java.lang.String str) {
        this.b.post(new java.lang.Runnable() { // from class: f.a.k.m0.e0
            @Override // java.lang.Runnable
            public final void run() {
                this.a.v0(i, str);
            }
        });
    }

    @Override // f.a.i.h.c
    public void onMusicInfo(net.easyconn.ecsdk.ECTypes.ECAppMusicInfo eCAppMusicInfo) {
    }

    @Override // f.a.i.h.g
    public void onOTAUpdateRequestDownload(net.easyconn.ecsdk.ECTypes.ECOTAUpdateSoftware[] eCOTAUpdateSoftwareArr, int i) {
    }

    @Override // f.a.i.h.c
    public void onPageStatusChanged(int i, int i2, int i3) {
    }

    @Override // f.a.i.h.c
    public void onParallelWorldAudioSource(int i) {
    }

    @Override // f.a.i.h.c
    public void onPhoneBluetoothState(int i, java.lang.String str) {
    }

    @Override // f.a.c
    public void onStart() {
    }

    @Override // f.a.i.h.c
    public void q(boolean z, boolean z2) {
    }

    public void q0(android.content.Context context, f.a.i.i.q qVar) {
        if (qVar.T) {
            return;
        }
        f.a.i.i.m mVar = qVar.b;
        if (mVar != null) {
            mVar.q = this;
        }
        qVar.C = this;
        this.f3108e = f.a.i.l.a.s(context);
        qVar.z(f.a.i.l.a.k(context), f.a.i.l.a.q(context), net.easyconn.ecsdk.ECTypes.ECOTAUpdateCheckMode.EC_OTA_CHECK_ONLY_LOCAL);
        if (this.f3107d) {
            return;
        }
        final java.lang.String strZ = f.a.i.l.a.z(context);
        e.a.c.a.a.w("checkLocalUpdatePackageAvailable packageFile = ", strZ);
        if (strZ != null) {
            this.b.post(new java.lang.Runnable() { // from class: f.a.k.m0.f0
                @Override // java.lang.Runnable
                public final void run() {
                    this.a.s0(strZ);
                }
            });
        }
    }

    public final void r0() {
        e.a.c.a.a.C(e.a.c.a.a.o("connectionError isConnectionErrorShowed = "), this.f3110g);
        if (this.f3110g) {
            return;
        }
        android.os.Handler handler = this.b;
        final f.a.k.k0.j jVar = this.a;
        jVar.getClass();
        handler.post(new java.lang.Runnable() { // from class: f.a.k.m0.h0
            @Override // java.lang.Runnable
            public final void run() {
                ((f.a.k.l0.a4) jVar).d0();
            }
        });
        this.f3110g = true;
    }

    public /* synthetic */ void s0(java.lang.String str) {
        ((f.a.k.l0.a4) this.a).g0(str, null);
    }

    @Override // f.a.i.h.c
    public void t(long j, long j2, long j3, long j4) {
    }

    public /* synthetic */ void t0(net.easyconn.ecsdk.ECTypes.ECOTAUpdateSoftware eCOTAUpdateSoftware) {
        ((f.a.k.l0.a4) this.a).g0(eCOTAUpdateSoftware.packagePath, eCOTAUpdateSoftware.vcDetail);
    }

    @Override // f.a.i.h.g
    public void u(net.easyconn.ecsdk.ECTypes.ECOTAUpdateSoftware[] eCOTAUpdateSoftwareArr, int i, net.easyconn.ecsdk.ECTypes.ECOTAUpdateSoftware[] eCOTAUpdateSoftwareArr2, int i2) {
        if (i2 <= 0) {
            return;
        }
        for (final net.easyconn.ecsdk.ECTypes.ECOTAUpdateSoftware eCOTAUpdateSoftware : eCOTAUpdateSoftwareArr2) {
            if (eCOTAUpdateSoftware.softwareId.equals(this.f3108e)) {
                this.b.post(new java.lang.Runnable() { // from class: f.a.k.m0.b0
                    @Override // java.lang.Runnable
                    public final void run() {
                        this.a.t0(eCOTAUpdateSoftware);
                    }
                });
                return;
            }
        }
    }

    public /* synthetic */ void u0(int i, java.lang.String str) {
        ((f.a.k.l0.a4) this.a).e0(i, str);
    }

    @Override // f.a.i.h.c
    public void v(int i) {
    }

    public /* synthetic */ void v0(int i, java.lang.String str) {
        ((f.a.k.l0.a4) this.a).f0(i, str);
    }

    public /* synthetic */ void w0() {
        this.a.t();
    }

    @Override // f.a.i.h.c
    public void x(boolean z) {
    }

    public /* synthetic */ void x0() {
        f.a.k.k0.j jVar = this.a;
        if (jVar != null) {
            jVar.e();
            this.a.I();
        }
    }

    public void y0() {
        if (this.a != null) {
            e.e.a.g.a("statusTipConnectingAtTime start");
            this.b.post(new java.lang.Runnable() { // from class: f.a.k.m0.d0
                @Override // java.lang.Runnable
                public final void run() {
                    this.a.w0();
                }
            });
            this.b.postAtTime(new java.lang.Runnable() { // from class: f.a.k.m0.g0
                @Override // java.lang.Runnable
                public final void run() {
                    this.a.x0();
                }
            }, h, android.os.SystemClock.uptimeMillis() + 10000);
        }
    }
}
